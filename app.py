...
def get_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS phone_activity (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        app_name TEXT NOT NULL, opened_at TEXT NOT NULL)""")
    return conn
...
def report():
    ...
    conn.execute("DELETE FROM phone_activity WHERE id NOT IN (SELECT id FROM phone_activity ORDER BY opened_at DESC LIMIT 100)")
...
def activity_summary():
    ...
    rows = conn.execute("SELECT app_name, opened_at FROM phone_activity ORDER BY opened_at DESC LIMIT 100").fetchall()
    conn.close()
    if not rows: return jsonify({"last_active": None, "recent_apps": [], "count": 0})
